...
available_setting = {
    ...
    "group_chat_in_plugin_mode": False,  # 是否让所有群聊消息进入插件系统（True=所有消息都经过插件，由插件或大模型处理；False=需要前缀/关键词/@才触发）
    ...
    # OpenAI Proxy配置 (支持OpenAI兼容的API代理服务)
    "openai_proxy_api_base": "https://api.example.com/v1",
    "openai_proxy_api_key": "",
    "openai_proxy_model": "gpt-4o",  # OpenAI Proxy使用的模型
    "openai_proxy_vision_model": "gpt-4o",  # OpenAI Proxy识图模型
    "openai_proxy_image_generation_model": "FLUX.1-schnell",  # OpenAI Proxy生图模型
    "openai_proxy_video_generation_model": "Grok-3",  # OpenAI Proxy生成视频模型
    "openai_proxy_video_duration": 5,  # 视频时长（秒）
    "openai_proxy_video_aspect_ratio": "16:9",  # 视频宽高比
    "video_create_prefix": ["grok", "生成视频", "grok视频"],  # 视频生成触发前缀
    # coze配置
    ...
    # 通用模型配置（兼容多种OpenAI格式API）
    "vision_model": "",  # 识图模型，留空则使用model配置
    "image_generation_model": "dall-e-3",  # 图片生成模型
    "video_generation_model": "",  # 视频生成模型
    "video_duration": 5,  # 视频时长（秒）
    "video_aspect_ratio": "16:9",  # 视频宽高比
    # 图像模型设置
