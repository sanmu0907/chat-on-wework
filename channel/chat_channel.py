...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        if ctype == ContextType.TEXT:
            # 检测是否是引用图片的识图请求（支持群聊和单聊）
            if "「" in content and "[图片]" in content:
                # 格式：「发送者：[图片]」\n- - - - - - - - - - - - - - -\n识图
                quote_pattern = r'「([^：]+)：\[图片\]」'
                quote_match = re.search(quote_pattern, content)
                if quote_match:
                    quoted_user_nickname = quote_match.group(1).strip()
                    logger.info(f"[chat_channel] 检测到引用图片消息，被引用者: {quoted_user_nickname}")

                    # 解析是否指定了历史图片
                    # 支持格式: "识图 倒数第2张", "识图 历史2", "识图 第2张"
                    skip_latest = 0  # 默认返回最新图片
                    skip_patterns = [
                        r'倒数第(\d+)张',  # 倒数第N张
                        r'历史(\d+)',      # 历史N
                        r'第(\d+)张',      # 第N张
                    ...
                    for pattern in skip_patterns:
                        skip_match = re.search(pattern, content)
                        if skip_match:
                            skip_num = int(skip_match.group(1))
                            skip_latest = skip_num - 1  # 第1张 = skip_latest 0, 第2张 = skip_latest 1
                            logger.info(f"[chat_channel] 检测到历史图片请求: 第{skip_num}张 (skip_latest={skip_latest})")
                            break
                    ...
                    # 处理单聊场景
                    if not is_group:
                        logger.info(f"[chat_channel] 单聊引用识图，直接使用session_id作为user_id")
                        session_id = context["session_id"]
                        quoted_user_id = session_id  # 单聊中，对方就是 session_id
                        room_id = None  # 单聊没有room_id
                    ...
                    # 处理群聊场景
                    elif msg and hasattr(msg, "other_user_id"):
                        # 读取群成员缓存
                        try:
                            import json
                            room_members_file = os.path.join(os.getcwd(), "tmp", "wework_room_members.json")
                            if os.path.exists(room_members_file):
                                with open(room_members_file, "r", encoding="utf-8") as f:
                                    room_members_data = json.load(f)
            ...
            if context.get("isgroup", False):  # 群聊
                # 检查是否启用"插件模式"（所有消息都进入插件系统）
                plugin_mode = conf().get("group_chat_in_plugin_mode", False)
                ...
                if context["msg"].to_user_id != context["msg"].actual_user_id:
                    ...
                    # 优先检查前缀，有前缀就触发（不需要关键词）
                    elif match_prefix is not None:
                        flag = True
                        content = content.replace(match_prefix, "", 1).strip()
                    # 如果没有前缀，检查是否包含关键词
                    elif match_contain is not None:
                        flag = True
            ...
            # 只有在未转换为IMAGE时才设置为TEXT（避免覆盖引用图片识图的IMAGE类型）
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if not e_context.is_pass():
            ...
            if context.type == ContextType.TEXT or context.type == ContextType.IMAGE:  # 文字和识图消息
            ...
            # 图片消息缓存，用于图生视频等场景
                ...
                # 缓存图片路径，供后续的grok命令使用（图生视频）
                session_id = context["session_id"]
                image_data = {
                    "path": context.content,
                    "msg": context.get("msg")
                }
                memory.USER_IMAGE_CACHE[session_id] = image_data

                # 保存到本地文件缓存，用于支持引用消息识图（3天有效期）
                msg = context.get("msg")
                if msg and hasattr(msg, "actual_user_id") and hasattr(msg, "other_user_id"):
                    image_cache_mgr = get_image_cache_manager()
                    cached_path = image_cache_mgr.save_image(
                        user_id=msg.actual_user_id,
                        room_id=msg.other_user_id,
                        image_path=context.content
                    )
                    if cached_path:
                        logger.info(f"[chat_channel] 图片已缓存到本地: {cached_path}")
