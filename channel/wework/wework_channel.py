...
class WeworkChannel(ChatChannel):
    ...
    def startup(self):
        ...
        if not contacts or not rooms:
            logger.warning("获取contacts或rooms失败，尝试从缓存文件加载")
            cache_contacts_path = os.path.join(directory, 'wework_contacts.json')
            cache_rooms_path = os.path.join(directory, 'wework_rooms.json')

            if not contacts and os.path.exists(cache_contacts_path):
                try:
                    with open(cache_contacts_path, 'r', encoding='utf-8') as f:
                        contacts = json.load(f)
                    logger.info(f"成功从缓存加载contacts，共{len(contacts.get('external_contact_list', []))}个联系人")
                except Exception as e:
                    logger.error(f"加载contacts缓存失败: {e}")

            if not rooms and os.path.exists(cache_rooms_path):
                try:
                    with open(cache_rooms_path, 'r', encoding='utf-8') as f:
                        rooms = json.load(f)
                    logger.info(f"成功从缓存加载rooms，共{len(rooms.get('room_list', []))}个群组")
                except Exception as e:
                    logger.error(f"加载rooms缓存失败: {e}")

            # 如果缓存也加载失败，则退出
            if not contacts or not rooms:
                logger.error("获取contacts或rooms失败，且缓存文件不存在或加载失败，程序退出")
                ntwork.exit_()
                sys.exit(0)
        else:
            # 获取成功，更新缓存文件
            ...
            logger.info("成功更新contacts和rooms缓存文件")
        # 创建一个空字典来保存结果
        result = {}
    ...
    def handle_group(self, cmsg: ChatMessage):
        if cmsg.ctype == ContextType.VOICE:
            ...
            # 自动缓存群聊中的图片，用于引用识图功能
            try:
                if hasattr(cmsg, 'actual_user_id') and hasattr(cmsg, 'other_user_id'):
                    user_id = cmsg.actual_user_id
                    room_id = cmsg.other_user_id
                    image_path = cmsg.content

                    # 等待图片下载完成
                    if hasattr(cmsg, 'prepare'):
                        cmsg.prepare()

                    # 缓存图片
                    if os.path.exists(image_path):
                        image_cache_mgr = get_image_cache_manager()
                        cached_path = image_cache_mgr.save_image(user_id, room_id, image_path)
                        if cached_path:
                            logger.info(f"[WX] 群聊图片已自动缓存: user_id={user_id}, room_id={room_id}")

                    else:
                        logger.warning(f"[WX] 图片文件不存在，无法缓存: {image_path}")
            except Exception as e:
                logger.error(f"[WX] 缓存群聊图片失败: {str(e)}")
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if reply.type == ReplyType.TEXT or reply.type == ReplyType.TEXT_:
            ...
            if isinstance(image_storage, str):
                # 如果是字符串路径，直接发送
                logger.info("[WX] Preparing to send image from path: {}".format(image_storage))
                if os.path.exists(image_storage):
                    result = wework.send_image(receiver, file_path=image_storage)
                    logger.info("[WX] sendImage result={}, receiver={}".format(result, receiver))

                    # 如果是群聊消息，将机器人发送的图片也缓存起来，以便后续识图
                    if context.get("isgroup", False) and result:
                        try:
                            bot_user_id = self.user_id  # 机器人自己的user_id
                            room_id = receiver  # 群组ID
                            image_cache_mgr = get_image_cache_manager()
                            image_cache_mgr.save_image(bot_user_id, room_id, image_storage)
                            logger.info(f"[WX] 机器人发送的图片已缓存: user_id={bot_user_id}, room_id={room_id}")
                        except Exception as e:
                            logger.error(f"[WX] 缓存机器人发送的图片失败: {str(e)}")
                else:
                    logger.error("[WX] Image file not found: {}".format(image_storage))
                    wework.send_text(receiver, "图片文件不存在")
            else:
                # 如果是文件对象，需要读取并保存为临时文件
                image_storage.seek(0)
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as temp:
                    ...
                    temp.write(data)
                # Send the image
                logger.info("[WX] Preparing to send image, temp_path={}, size={} bytes".format(temp_path, len(data)))
                result = wework.send_image(receiver, file_path=temp_path)
                logger.info("[WX] sendImage result={}, receiver={}".format(result, receiver))

                # Wait a bit before removing the temporary file to ensure WeWork has read it
                time.sleep(2)
                # Remove the temporary file
                try:
                    os.remove(temp_path)
                    logger.debug("[WX] Temporary image file removed: {}".format(temp_path))
                except Exception as e:
                    logger.warning("[WX] Failed to remove temp file: {}".format(e))
        elif reply.type == ReplyType.VIDEO:  # 从BytesIO读取视频
            video_storage = reply.content

            # 判断是文件对象还是文件路径
            if isinstance(video_storage, str):
                # 如果是字符串路径，直接发送
                logger.info("[WX] Preparing to send video from path: {}".format(video_storage))
                if os.path.exists(video_storage):
                    result = wework.send_video(receiver, video_storage)
                    logger.info("[WX] sendVideo result={}, receiver={}".format(result, receiver))
                else:
                    logger.error("[WX] Video file not found: {}".format(video_storage))
                    wework.send_text(receiver, "视频文件不存在")
            else:
                # 如果是文件对象，需要读取并保存为临时文件
                video_storage.seek(0)
                # Read data from video_storage
                data = video_storage.read()
                # Create a temporary file with .mp4 extension
                with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as temp:
                    temp_path = temp.name
                    temp.write(data)
                # Send the video
                logger.info("[WX] Preparing to send video, temp_path={}, size={} bytes".format(temp_path, len(data)))
                result = wework.send_video(receiver, temp_path)
                logger.info("[WX] sendVideo result={}, receiver={}".format(result, receiver))
                # Wait a bit before removing the temporary file to ensure WeWork has read it
                time.sleep(3)
                # Remove the temporary file
                try:
                    os.remove(temp_path)
                    logger.debug("[WX] Temporary video file removed: {}".format(temp_path))
                except Exception as e:
                    logger.warning("[WX] Failed to remove temp file: {}".format(e))
